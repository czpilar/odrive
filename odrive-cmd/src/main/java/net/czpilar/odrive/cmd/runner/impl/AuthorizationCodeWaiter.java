...
public class AuthorizationCodeWaiter {
    ...
    public Optional<String> getCode() {
        ...
        try {
            System.out.println("Waiting " + WAIT_TIME + " minutes for receiving authorization code to authorize the oDrive application...");
            code = doWait();
        } catch (IOException | InterruptedException e) {
            throw new CommandLineException(e);
        } finally {
            if (code.isEmpty()) {
                System.out.println("oDrive application was not authorized due to expired waiting time.");
            }
        }
        return code;
    }
    ...
    private Optional<String> doWait() throws IOException, InterruptedException {
        HttpServer server = HttpServer.create(new InetSocketAddress(oDriveSetting.getRedirectUriPort()), 0);
        AuthorizationCodeWaiterHandler handler = new AuthorizationCodeWaiterHandler();
        server.createContext(oDriveSetting.getRedirectUriContext(), handler);
        server.start();
